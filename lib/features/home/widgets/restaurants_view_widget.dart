...
class RestaurantView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Calculate delivery charge based on your logic
    double deliveryCharge = calculateDeliveryCharge(restaurant); // Implement this function based on your logic
    return Container(
            ...
            Container(
              ...
              child: ClipRRect(
                ...
                child: CustomImageWidget(
                  ...
                  fit: BoxFit.cover, height: 145, width: double.infinity,
                  isRestaurant: true,
                ),
              ),
            ),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      restaurant.freeDelivery!
                        ...
                        child: Row(
                          children: [
                            Image.asset(Images.deliveryIcon, height: 30, width: 30), // Delivery man icon
                            const SizedBox(width: Dimensions.paddingSizeExtraSmall),
                            Text(
                              '${deliveryCharge.toString()} Dt', // Assuming you have a currency string
                              style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeExtraSmall),
                            ),
                          ],
                        ),
  ...
  double calculateDeliveryCharge(Restaurant restaurant) {
    // Add your delivery charge calculation logic here.
    // For example, this could be based on the distance from the restaurant,
    // minimum order value, or any other factors.

    // Placeholder logic:
    double baseCharge = 2.5; // Base delivery charge
    double distance = Get.find<RestaurantController>().getRestaurantDistance(
      LatLng(double.parse(restaurant.latitude!), double.parse(restaurant.longitude!)),
    );

    // Example of modifying the charge based on distance
    if (distance > 5) {
      return baseCharge + (distance - 5) * 0.5; // 0.5Dt for every km over 5km
    }

    return baseCharge; // Return base charge if within 5km
  }
}
