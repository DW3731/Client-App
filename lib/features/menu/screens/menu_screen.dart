...
class _MenuScreenState extends State<MenuScreen> {
                    ...
                    Container(
                      ...
                      child: ClipOval(
                        child: CustomImageWidget(
                          ...
                          width: 60,
                          fit: BoxFit.cover,
                          imageColor: isLoggedIn ? null : Theme.of(context).primaryColor,
                        ),
                      ),
                    ),
                          ...
                          [
                            ...
                            if (Get.find<SplashController>().configModel!.loyaltyPointStatus == 1)
                              PortionWidget(
                                ...
                                suffix: !isLoggedIn ? null : '${profileController.userInfoModel?.loyaltyPoint ?? '0'} ${'points'.tr}',
                              ),
                            if (Get.find<SplashController>().configModel!.customerWalletStatus == 1)
                              PortionWidget(
                                ...
                                suffix: !isLoggedIn ? null : PriceConverter.convertPrice(profileController.userInfoModel?.walletBalance ?? 0),
                              ),
                          ],
                        ),
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Container(
                                      ...
                                      child: Icon(
                                        Get.find<AuthController>().isLoggedIn() ? Icons.logout : Icons.login,
                                        size: 16,
                                        color: Colors.black,
                                      ),
                                    ),
  ...
  Widget _buildSection(BuildContext context, List<Widget> widgets) {
    return Column(
      children: widgets.map((widget) => Padding(
        padding: const EdgeInsets.symmetric(vertical: Dimensions.paddingSizeSmall),
        child: widget,
      )).toList(),
    );
