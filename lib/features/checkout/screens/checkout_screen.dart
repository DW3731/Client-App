...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  /*double? _getDeliveryCharge({required Restaurant? restaurant, required CheckoutController checkoutController, bool returnDeliveryCharge = true, bool returnMaxCodOrderAmount = false}) {
    ...
    // Step 1: Start with the minimum charge
    double deliveryCharge = minimumCharge;

    // Step 2: Add extra charges for distances over 5 km
    if (distance > 5) {
      deliveryCharge += perKmCharge * (distance - 5);
    }

    // Step 3: Ensure delivery charge does not exceed the maximum charge
    if (maximumCharge != null && deliveryCharge > maximumCharge) {
      deliveryCharge = maximumCharge;
    }
    ...
    if (restaurant.selfDeliverySystem == 0 && checkoutController.extraCharge != null) {
      extraChargeForToolTip = checkoutController.extraCharge!;
      deliveryCharge += checkoutController.extraCharge!;
      charge += checkoutController.extraCharge!;
    }

    // Bad weather charge logic
    if (restaurant.selfDeliverySystem == 0 && zoneData.increasedDeliveryFeeStatus == 1) {
      badWeatherChargeForToolTip = (deliveryCharge * (zoneData.increasedDeliveryFee! / 100));
      deliveryCharge += (deliveryCharge * (zoneData.increasedDeliveryFee! / 100));
      charge += charge * (zoneData.increasedDeliveryFee! / 100);
    }

    // Free delivery logic based on distance
    if (restaurant.selfDeliverySystem == 0 && Get.find<SplashController>().configModel!.freeDeliveryDistance != null && Get.find<SplashController>().configModel!.freeDeliveryDistance! >= distance) {
      deliveryCharge = 0;
      charge = 0;
    }
    ...
    // Return COD maximum amount if requested
    double? maxCodOrderAmount;
    if (zoneData.maxCodOrderAmount != null) {
      maxCodOrderAmount = zoneData.maxCodOrderAmount;
    }
